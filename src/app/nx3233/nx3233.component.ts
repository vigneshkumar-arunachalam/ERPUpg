...
export class Nx3233Component implements OnInit {
  ...
  AssignCustomerSingle(licenceID: any) {
    ...
    console.log(" this.checkedValuesLicense-ob edit", this.checkedValuesLicense)
  ...
  assignCustomerSave_single() {
    ...
    if (!this.checkedValuesLicense) { // Checks for undefined, null, false, '', 0
      api_postUPd.checkedValues = this.checkedValuesLicense;
    } else {
     console.log("inside-if", this.selectedResellerCommIds_unpaid);
...
console.log("this.myFiles",this.myFiles);
  ...
  assignCustomerSave() {
    console.log("this.checkedValuesLicense", this.checkedValuesLicense);
    ...
    api_postUPd.user_id = localStorage.getItem('erp_c4c_user_id');
    if (this.customerID == '' || this.customerID == 'undefined' || this.customerID == undefined || this.customerID == null) {
      iziToast.error({
        message: "Customer Value Missing",
        position: 'topRight'
      });
      this.spinner.hide();
      return false;
