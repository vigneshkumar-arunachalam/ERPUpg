...
@Component({
  ...
  styleUrls: ['./salaryentry.component.css']
})
...
console.log("this.tempVariable",this.Purchase_Order);
    this.Purchase_Order.forEach((item: { selected: boolean; salaryId: any;  }) => {
      item.selected = this.selectAllCheckbox; // Update each item's selected state

      if (item.selected) {
        this.selectedResellerCommIds_unpaid.push(item.salaryId);
  ...
  updateSelected_assignLicense(item: any) {
    if (item.selected) {
      ...
      const index = this.selectedResellerCommIds_unpaid.indexOf(item.salaryId);
      if (index > -1) {
  ...
  PurchaseOrderList(data: any) {
    ...
    api_req.moduleType = "hr";
    api_req.api_url = "hr/salaryList"
    api_req.api_type = "web";
    api_req.access_token = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..NaymQDSiON2R3tKICGNpj6hsQfg9DGwEcZzrJcvsqbI";
    api_deliveryOrder.action = "hr/salaryList";
  ...
  goAddSalary() {
    this.router.navigate(['/addSalaryEntry'])
  }
  editSalaryEntry(salaryId:any){
    this.router.navigate(['/editSalaryEntry'], {
      queryParams: {
        salaryId: salaryId,
      }
    });
  ...
  rowDelete(salaryId: any) {
    this.rowSalaryID=salaryId;
        Swal.fire({
          ...
          if (result.value) {
            ...
            let api_deliveryOrder: any = new Object();
            api_req.moduleType = "hr";
            api_req.api_url = "hr/delete_salaryDetails"
            api_req.api_type = "web";
            api_req.access_token = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..NaymQDSiON2R3tKICGNpj6hsQfg9DGwEcZzrJcvsqbI";
            api_deliveryOrder.action = "hr/delete_salaryDetails";
            api_deliveryOrder.user_id = localStorage.getItem("erp_c4c_user_id");
            api_deliveryOrder.salaryId = this.rowSalaryID;
            api_req.element_data = api_deliveryOrder;
        ...
        if(this.selectedResellerCommIds_unpaid.length>0  ){
          ...
          Swal.fire({
            ...
            if (result.value) {
              ...
              api_deliveryOrder.action = "hr/delete_salaryDetails";
              api_deliveryOrder.user_id = localStorage.getItem("erp_c4c_user_id");
              api_deliveryOrder.salaryId = formattedIds;
              api_req.element_data = api_deliveryOrder;
