...
@Component({
  selector: 'app-add-decarepi',
  templateUrl: './add-decarepi.component.html',
  styleUrls: ['./add-decarepi.component.css'],
})
export class AddDecarepiComponent implements OnInit {
  ...
  keyPress(event: any, i: any) {
    ...
    if (this.finalTax > 0) {
      ...
      console.log('this.finalTax keypress', this.finalTax);
    }
    if (this.finalTax2 > 0) {
      ...
      // console.log('this.finalTax2 keypress',this.finalTax2);
    }
  }
  ...
  searchCustomer_selectDropdownData(data: any) {
    ...
    this.serverService.sendServer(api_req).subscribe((response: any) => {
      ...
      if (response.status == true) {
        ...
        if (
          response.customer_details.ship_to == '' ||
          response.customer_details.ship_to == null
        ) {
        ...
        if (
          response.customer_details.ship_customerAddress1 == '' ||
          response.customer_details.ship_customerAddress1 == null
        ) {
        ...
        if (
          response.customer_details.ship_customerAddress2 == '' ||
          response.customer_details.ship_customerAddress2 == null
        ) {
        ...
        if (
          ship_address_str3 != '' &&
          response.customer_details.ship_state != '' &&
          response.customer_details.ship_state != null
        ) {
          ship_address_str3 =
        ...
        if (
          ship_address_str3 != '' &&
          response.customer_details.ship_country != '' &&
          response.customer_details.ship_country != null
        ) {
          ship_address_str3 =
  ...
  save($event: MouseEvent) {
    ...
    api_req.moduleType = 'dcare_proforma';
    api_req.api_url = 'dcare_proforma/insert_proforma_invoice';
    api_req.api_type = 'web';
    api_req.access_token =
      'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..NaymQDSiON2R3tKICGNpj6hsQfg9DGwEcZzrJcvsqbI';
    ...
    if (
      this.addPI_section1.value.companyName === null ||
      this.addPI_section1.value.companyName === undefined
    ) {
      iziToast.warning({
    ...
    if (
      this.customerName_Data === null ||
      this.customerName_Data === undefined
    ) {
      iziToast.warning({
        message: 'Select Bill',
        position: 'topRight',
    ...
    api_savePI_req.mile_discount_display_state =
      this.mile_discount_display_state;
    ...
    // /// tax start ///
    const selectedTaxId = this.addPI_section3.get(
      'section3_gst_dropdown'
    )?.value;
    const selectedTax = this.getTaxValue(selectedTaxId);
    const selectedTaxId2 = this.addPI_section3.get(
      'section3_gst_dropdown_2'
    )?.value;
    const selectedTax2 = this.getTaxValue(selectedTaxId2);
    ...
    api_savePI_req.taxPer = selectedTax;
    api_savePI_req.taxId2 = this.addPI_section3.value.section3_gst_dropdown_2;
    api_savePI_req.taxAmt2 = this.addPI_section3.value.section3_taxAmt_txtbox_2;
    api_savePI_req.taxPer2 = selectedTax2;

    /// tax end ///
    api_savePI_req.shippingAmt =
      this.addPI_section3.value.section3_shipping_amt_txtbox;
    api_savePI_req.shippingName =
      this.addPI_section3.value.section3_shipping_amt_name_txtbox;
    api_savePI_req.add_name =
      this.addPI_section3.value.section3_bankingCharge_amt_name_txtbox;
    api_savePI_req.add_amt =
      this.addPI_section3.value.section3_bankingCharge_amt_txtbox;
    ...
    api_savePI_req.received_signature =
      this.addPI_section3.value.section3_receivedAuthorizedSignature;
  ...
  getTaxValue(taxId: string) {
    const selected = this.TaxDropdownList.find(
      (tax: { taxId: string }) => tax.taxId == taxId
    );
    return selected ? selected.percent_val : '';
  }
  ...
  invoiceAddSignature() {
    ...
    api_invoiceAddSignature_req.user_id =
      localStorage.getItem('erp_c4c_user_id');
    api_invoiceAddSignature_req.billerId =
  ...
  totalCalculate() {
    ...
    this.grossTotalTax2 = tax_amt_tot2;
    ...
    if (tax_amt_tot2 > 0) {
      tax_amt_tot2 =
        (parseFloat(this.tax_per_mod_2) *
          (parseFloat(this.grossTotalTax2) - parseFloat(this.finalDiscount))) /
        100;
    }
    ...
    if (this.finalTax2 == '' || isNaN(this.finalTax2)) {
      this.finalTax2 = 0;
    }
