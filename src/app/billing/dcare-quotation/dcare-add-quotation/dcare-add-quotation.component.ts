...
@Component({
  selector: 'app-dcare-add-quotation',
  templateUrl: './dcare-add-quotation.component.html',
  styleUrls: ['./dcare-add-quotation.component.css']
})
export class DcareAddQuotationComponent implements OnInit {
...
// console.log(response);
  ...
  saveQuotationEnquiry($event: MouseEvent) {
    ...
    if (this.radioSelectFooterChecked == true) {
     ...
     // console.log("this.addQuotationInvoice_section1.value.selectPDFTemplate", this.addQuotationInvoice_section1.value.selectPDFTemplate)
      ...
      api_req.api_url = "dcare_quotation/insert_quotation";
      api_req.api_type = "web";
      api_req.access_token = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..NaymQDSiON2R3tKICGNpj6hsQfg9DGwEcZzrJcvsqbI";
      ...
      //row-3
        ...
        // /// tax start ///
        const selectedTaxId = this.addQuotationInvoice_section3.get(
          'section3_gst_dropdown'
        )?.value;
        const selectedTax = this.getTaxValue(selectedTaxId);
        const selectedTaxId2 = this.addQuotationInvoice_section3.get(
      ...
      api_saveEnquiry_req.taxPer = selectedTax;
      api_saveEnquiry_req.taxAmt = this.addQuotationInvoice_section3.value.section3_taxAmt_txtbox;
     

      api_saveEnquiry_req.taxId2 = this.addQuotationInvoice_section3.value.section3_gst_dropdown_2;
      api_saveEnquiry_req.taxPer2 = selectedTax2;
      api_saveEnquiry_req.taxAmt2 = this.addQuotationInvoice_section3.value.section3_taxAmt_txtbox_2;
  ...
  totalCalculate_last() {
    ...
    this.grandTotal = ((parseFloat(this.grossTotal) + parseFloat(this.finalTax) + parseFloat(this.finalTax2) + parseFloat(this.shipping_amt)) - parseFloat(this.finalDiscount)).toFixed(2);
   // console.log("this.grandTotal", this.grandTotal);
  }
  ...
  keyPress(event: any, i: any) {
    ...
    if (this.finalTax2 > 0) {
      var tax = this.addQuotationInvoice_section3.value.section3_gst_dropdown_2;
      tax = (parseFloat(tax) * parseFloat(this.grossTotal) / 100).toFixed(2);
      if (this.grandTotal > 0) {
        this.grandTotal = this.grandTotal + parseFloat(tax);
      }
  ...
  getTaxCals_2() {
    ...
    var tax_id = this.addQuotationInvoice_section3.value.section3_gst_dropdown_2;
    var tax: any;
    let api_req: any = new Object();
    let api_data_req: any = new Object();
    this.finalDiscount = $('#finalDiscount_amt').val();
    ...
    this.serverService.sendServer(api_req).subscribe((response: any) => {
     ...
     // console.log(" this.finalTax", this.finalTax)
      $('#tax_amt_id_2').val(this.finalTax2);
