...
export class DupQuotationNewComponent implements OnInit {
  ...
  quotationList(data: any) {
    ...
    this.serverService.sendServer(api_req).subscribe((response: any) => {
      ...
      if (response) {
        // Swal.close();
        this.spinner.hide();
        if (response.quotation_details) {
          this.quotation_list = response.quotation_details;
        }

        this.response_total_cnt = response.total_cnt;
        if (response.quotation_permission_arr) {
  ...
  templateContentEmailDropdown(event: any) {
    ...
    this.serverService.sendServer(api_req).subscribe((response: any) => {
      ...
      this.mailContent = tinymce.get('tinyID_DC_qu').setContent("<p>" + this.messageContent + "</p>");
      if (response != '') {
        this.emailForm.patchValue({

          'Subject_Content': response.crm_subject_name,

          'tinyID_DC_qu': this.mailContent,
  ...
  sendMail() {
    ...
    this.msg_id = tinymce.get('tinyID_DC_qu').getContent();
