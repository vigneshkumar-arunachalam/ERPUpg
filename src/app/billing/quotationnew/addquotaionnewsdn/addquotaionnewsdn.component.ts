...
@Component({
  selector: 'app-addquotaionnewsdn',
  templateUrl: './addquotaionnewsdn.component.html',
  styleUrls: ['./addquotaionnewsdn.component.css']
})
export class AddquotaionnewsdnComponent implements OnInit {
  ...
  companyName_enquiryFromDetails: any;
  company_enquiryFromDetails: any;
  ...
  ngOnInit(): void {
    ...
    this.addQuotationInvoice_section3 = new FormGroup({
      ...
      'GrossTotalafterdiscount': new FormControl(null),
      ...
      'section3_tax_total': new FormControl(null),
 ...
 //   console.log("onFocusedProduct ", e)
  ...
  saveQuotationEnquiry($event: MouseEvent) {
    ...
    if (this.radioSelectFooterChecked == true) {
     ...
     // console.log("this.addQuotationInvoice_section1.value.selectPDFTemplate", this.addQuotationInvoice_section1.value.selectPDFTemplate)
      ...
      api_saveEnquiry_req.gross_total = $('#gross1').val();
      //row-2
      api_saveEnquiry_req.discount_amt_tot = $('#finalDiscount_amt').val();
      ...
      //row-5
      api_saveEnquiry_req.grand_total = $('#grand1').val();;
      //row-6
      ...
      for (let i = 0; i < addr.length; i++) {
       ...
       // console.log(addr[i].pd_quantity_txtbox1)
        ...
        addr[i].pd_gst_tax = $('#pd_gst_tax_' + i).val();
        addr[i].pd_gst_tax_value = $('#pd_gst_tax_value_' + i).val();
        addr[i].pd_netPrice_disc = $('#pd_netPrice_disc_' + i).val();
  ...
  totalCalculate() {
    ...
    if (this.finalDiscount == '' || this.finalDiscount == 'undefined' || this.finalDiscount == undefined) {
  ...
  // get_newGst_tax(i: any) {
  //   // alert($('#pd_gst_tax_0' ).val())
  //   var tax_id = $('#pd_gst_tax_' + i).val();
  ...
  //     var taxPercentage=response.percent_val;
  //     console.log("tax-id",tax_id);
  //     console.log("tax-corresponding Percentage value",taxPercentage);
  //     var net_total = parseFloat($('#pd_netPrice_disc' + i).val()); // Convert to number
  //     console.log("net_total-index",net_total);
  //     var net_total = parseFloat($('#pd_netPrice_disc' + i).val());
  //     var taxValu = (parseFloat(taxPercentage) * (net_total) / 100).toFixed(2);
  //     console.log("tax value after calc",taxValu);
  //     $('#pd_gst_tax_value_' + i).val(taxValu);
  //     var netTotalChange = net_total + parseFloat(taxValu); // Convert to number
  //     $('#pd_netPrice_disc' + i).val(netTotalChange)
  ...
  get_newGst_tax(i: any) {
    // this.i_value=i;
    var tax_id = $('#pd_gst_tax_' + this.i_value).val();
    this.spinner.show();
    ...
    //  console.log("tax response", response.percent_val);
     ...
     // console.log("isChecked_tax", isChecked_tax);
      if(isChecked_tax){
        ...
        this.netCalculation_wo_Tax();
      }

      if (tax_id == null) {
        $('#pd_gst_tax_' + this.i_value).val('');
        this.spinner.hide();
      }
  ...
  netCalculation_wo_Tax() {
    ...
    if (quantityElement.length && SPElement.length) { // Check if all elements exist
      this.spinner.hide();
      var quantity = quantityElement.val(); // Retrieve quantity value
  ...
  //   var enablePriceDiscont = $('#enablePriceDiscont').val()
  //   var disType = $('input:radio[name=discountTYpe]:checked').val();
  //   var final_tot = $('#pd_Total_' + this.i_value).val();
  //   $('#sub_discount_type_' + this.i_value).val(disType);
  //   var price: any;
  //   if (disType == 'per') {
  ...
  saveDiscount_new(i: any) {
   // console.log("coming");
    ...
    var disType = $('input:radio[name=discountTYpe]:checked').val();
    var final_tot = $('#pd_Total_' + this.i_value).val();
    var gstTaxValue = $('#pd_gst_tax_value_' + this.i_value).val(); // Get GST tax value
    $('#sub_discount_type_' + this.i_value).val(disType);
    var price: any;
    ...
    // Add GST tax value to price if not empty
    if (gstTaxValue) {
      price = parseFloat(price) + parseFloat(gstTaxValue);
    }
  ...
  //  console.log("final_tot", final_tot);
   ...
   // $('#finalDiscount_amt').val(price.toFixed(2)); // Round the final price to 2 decimal places
    $('#discountFormFinal').modal('hide');
    this.calculate_grosstotal_new();
    this.calculate_grandTotal_new();
    this.DiscountForm.reset();
  }
  ...
  //  console.log("do")
   ...
   //   console.log("netPriceValue", netPriceValue)
     ...
     //   console.log("gross result-inner-w/o tax", totalNetPrice);
      ...
      //   console.log("gross result-inner-with tax", totalgstTaxValue);
      // }
  ...
  //  console.log("shippingAmount_val", shippingAmount_val)
    ...
    if (isNaN(gross_val) && isNaN(gross_Discount_val) && isNaN(shippingAmount_val)) {
      grandTotal = ''; // If all values are not provided, set grand total to empty string
    } else {
      if (gross_val > 0 && gross_Discount_val === 0 && shippingAmount_val === 0) {
        grandTotal = gross_val.toString();
      } else if (gross_val > 0 && gross_Discount_val > 0 && shippingAmount_val === 0) {
        grandTotal = (gross_val - gross_Discount_val).toString();
      } else if (gross_val > 0 && gross_Discount_val > 0 && shippingAmount_val > 0) {
        grandTotal = (gross_val + shippingAmount_val - gross_Discount_val).toString();
      } else if (gross_val > 0 && gross_Discount_val === 0 && shippingAmount_val > 0) {
        grandTotal = (gross_val + shippingAmount_val).toString();
      } else if (shippingAmount_val > 0) {
        grandTotal = shippingAmount_val.toString();
      }
    }

    $('#grand1').val(grandTotal); // Set the calculated grand total or empty string to the input field with id 'grand1'
  }
  ...
  //   api_req.api_url = "quotation/get_tax_percent_val";
  //   api_req.api_type = "web";
  //   api_req.access_token = "YOUR_ACCESS_TOKEN_HERE";
  //   api_data_req.action = "get_tax_percent_val";
  //   api_data_req.tax_id = tax_id;
  //   api_req.element_data = api_data_req;
  ...
  //       if (quantityElement.length && SPElement.length && subDiscountElement.length) { // Check if all elements exist
  //           var quantity = parseFloat(quantityElement.val()); // Retrieve quantity value and parse as float
  //           var SP = parseFloat(SPElement.val()); // Retrieve selling price value and parse as float
  //           var subDiscountAmount = parseFloat(subDiscountElement.val()); // Retrieve sub discount value and parse as float

  //           console.log("quantity:", quantity);
  //           console.log("SP:", SP);
  //           console.log("Sub Discount Amount:", subDiscountAmount);

  //           if (!isNaN(quantity) && !isNaN(SP)) { // Check if quantity and SP are numbers
  //               var net_total_input = (quantity * SP) - subDiscountAmount; // Calculate net total input
  //               console.log("net_total-index input", net_total_input);

  //               if (!isNaN(net_total_input)) { // Check if calculation result is a valid number
  //                   var net_total = net_total_input; // Assign net_total_input directly
  //                   console.log("net_total-index", net_total);

  //                   $('#pd_netPrice_' + i).val(net_total); // Set net total as value for net price input

  //                   // Apply Discount
  //                   var enablePercentabeDiscont = $('#enablePercentabeDiscont').val();
  //                   var enablePriceDiscont = $('#enablePriceDiscont').val();
  //                   var disType = $('input:radio[name=discountTYpe]:checked').val();
  //                   var final_tot = $('#pd_Total_' + i).val();
  //                   $('#sub_discount_type_' + i).val(disType);
  //                   var price: any;
  //                   if (disType == 'per') {
  //                       if (enablePercentabeDiscont != '') {
  //                           price = (parseFloat(enablePercentabeDiscont) * parseFloat(final_tot) / 100).toFixed(2);
  //                           $('#sub_discount_' + i).val(price);
  //                           $('#sub_discount_val_' + i).val(enablePercentabeDiscont);
  //                           price = final_tot - price;
  //                       } else {
  //                           $('#sub_discount_' + i).val('');
  //                           $('#sub_discount_val_' + i).val('');
  //                           price = final_tot;
  //                       }
  //                   } else {
  //                       price = final_tot - enablePriceDiscont;
  //                       $('#sub_discount_' + i).val(enablePriceDiscont);
  //                       $('#sub_discount_val_' + i).val(enablePriceDiscont);
  //                   }
  //                   $('#pd_netPrice_disc_' + i).val(price); // Set net price after discount
  ...
  //                       if (!isNaN(taxPercentage)) { // Check if tax percentage is a valid number
  //                           var taxValue = (taxPercentage * net_total / 100).toFixed(2);
  //                           console.log("tax value after calc", taxValue);
  //                           $('#pd_gst_tax_value_' + i).val(taxValue); // Set tax value

  //                           var netTotalChange = net_total + parseFloat(taxValue); // Calculate net total including tax
  //                           $('#pd_netPrice_disc_' + i).val(netTotalChange); // Update net total after discount with tax
  //                           console.log("net total value after calc", netTotalChange);
  //                       } else {
  //                           console.log("Error: Invalid tax percentage");
  //                       }
  //                   });

  //                   // Proceed with remaining code here if needed
  //               } else {
  //                   console.log("Error: Invalid calculation result for net_total_input");
  //               }
  //           } else {
  //               console.log("Error: Input for quantity or SP is not a number");
  //           }
  //       } else {
  //           console.log("Error: Quantity, SP, or sub discount elements not found");
  //       }
  ...
  getTaxCals() {
    ...
    var b = $('#finalDiscount_amt').val();

    var ted: any;
    ted = $('#GrossTotalafterdiscount').val();
  ...
  set_display_none(cnt: any) {
    if ($('#pd_split_' + cnt).prop('checked') == true) {
      ...
      // $('#pd_gst_tax_' + cnt).fadeOut(1000);
      // $('#pd_gst_tax_value_' + cnt).fadeOut(1000);
      // $('#pd_netPrice_disc_' + cnt).fadeOut(1000);
      ...
      // $('#pd_gst_tax_' + cnt).val('');
      // $('#pd_gst_tax_value_' + cnt).val('');
      // $('#pd_netPrice_disc_' + cnt).val('');
      ...
      // $('#pd_gst_tax_' + cnt).fadeIn(1000);
      // $('#pd_gst_tax_value_' + cnt).fadeIn(1000);
      // $('#pd_netPrice_disc_' + cnt).fadeIn(1000);
    }
  }
