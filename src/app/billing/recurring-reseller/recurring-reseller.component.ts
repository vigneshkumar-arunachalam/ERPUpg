...
@Component({
  selector: 'app-recurring-reseller',
  templateUrl: './recurring-reseller.component.html',
  styleUrls: ['./recurring-reseller.component.css']
})
export class RecurringResellerComponent implements OnInit {
  PI_list: any;
  user_ids: any;
  ...
  addResellerPayment() {
    ...
    $('#RPRR_AddResellerPaymentID').modal('show');
  }
  searchResellerPayment() {
    $('#RPRR_searchResellerPaymentID').modal('show');
  }
  ...
  getResellerPaymentdetails(data: any, customerid: any) {
   ...
   // console.log("customer id-assign in list---this.custID", this.custID)
    ...
    api_req.moduleType = "reseller";
    api_req.api_url = "reseller/RecurringResellerDetails";
    api_req.api_type = "web";
    api_req.access_token = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..NaymQDSiON2R3tKICGNpj6hsQfg9DGwEcZzrJcvsqbI";
    api_getReseller.action = "RecurringResellerDetails";
  ...
  UpdateResellerPayment() {
    ...
    this.serverService.sendServer(api_req).subscribe((response: any) => {
      ...
      if (response != '') {
        ...
        this.spinner.hide();
        $('#RPRR_ResellerPaymentID').modal('hide');
  ...
  saveResellerPayment() {
    ...
    this.serverService.sendServer(api_req).subscribe((response: any) => {
      ...
      if (response != '') {
        ...
        this.spinner.hide();
        $('#RPRR_AddResellerPaymentID').modal('hide');
...
this.spinner.show();
    ...
    this.serverService.sendServer(api_req).subscribe((response: any) => {
      ...
      if (response != '') {
        ...
        $('#RPRR_PaymentIIFormID').modal('hide');
        this.spinner.hide();
  ...
  ResellerPaySharePermissionEdit(reseller_id: any, i: any) {
    ...
    $("#faqheaduu" + i).modal("hide");
    this.spinner.show();
    $('#RPRR_SharePermissionFormID').modal('show');
  ...
  ResellerPaySharePermissionUpdate() {
    ...
    this.serverService.sendServer(api_req).subscribe((response: any) => {
      ...
      if (response.result == true) {
        ...
        $('#RPRR_SharePermissionFormID').modal('hide');
  ...
  searchResPay() {
    ...
    this.serverService.sendServer(api_req).subscribe((response: any) => {
      ...
      if (response != '') {
        ...
        this.commission_amt = response.commission_amt;
        $('#RPRR_searchResellerPaymentID').modal('hide');
  ...
  sendLandscapeMail() {
    ...
    this.serverService.sendServer(api_req).subscribe((response: any) => {
      ...
      if (response.status == true) {
        ...
        $("#RPRR_landscapeEmailFormId").modal("hide");
        Swal.close();
        iziToast.success({
  ...
  update_WFA_ResellerCommission() {
    ...
    this.serverService.sendServer(api_req).subscribe((response: any) => {
      this.spinner.hide();
      $('#RPRR_ResellerCommissionFormId').modal('hide');
